...
;***set parameters
	...
	st = 0 
	w = indgen(len)
	if pl eq 1 or keyword_set(excl) then plot,w,s,/xsty,/ynoz
	mbin = fltarr(sbin) 
	mbw = mbin
...
;***catch the end if not already gotten
	...
	if pl eq 1 then oplot,mbw,mbin,psym=6,co=!red
	;oplot,wc,sc,psym=1
;***polynomial or spline fit to continuum
	...
	if nord le 6 then begin
		...
		cf = robust_poly_fit(x,y,npl) 		;call poly_fit(intrinsic)
		c = poly(wf,cf) 		;call poly(user written)
		;print,cf
	endif else begin
		wf = float(w)
;		c = spline(x,y,wf)		;call spline
		c = dspline(x,y,wf)		;fast FORTRAN spline
	endelse
	if pl eq 1 then oplot,w,c,co=!green
	if pl eq 2 then begin
	   !p.multi=[0,1,2]
	   plot,w,s,/ynoz
	   if keyword_set(excl) then oplot,wsec,ssec-.75e5,lines=1
	   oplot,mbw,mbin,psym=6
	   oplot,w,c
