pro occultquad_vec,z0,u1,u2,p0,muo1,mu0,lambdae,lambdad,etad
; NAME:
;   OCCULTQUAD
;
; PURPOSE: 
...
;   cite Mandel & Agol (2002) and Eastman et al (in prep) if you make use
;   of this routine in your research.  Please report errors or bugs to
;   agol@astro.washington.edu and jdeast@astronomy.ohio-state.edu
;
; Limb darkening has the form:
;  I(r)=[1-u1*(1-sqrt(1-(r/rs)^2))-u2*(1-sqrt(1-(r/rs)^2))^2]/(1-u1/3-u2/6)/pi
; 
; CALLING SEQUENCE:
;    occultquad, z0, u1, u2, p0, muo1, mu0
;
; INPUTS:
;
...
; OPTIONAL OUTPUTS:
;
;    mu0     - fraction of flux at each z0 for a uniform source, also
;              used for calculation of another transit with different limb
;              darkening coefficients (25x faster than recalculating fully)
;    lambdad - for calculation of another transit with different limb
;              darkening coefficients (25x faster than recalculating fully)
;    etad    - for calculation of another transit with different limb
;              darkening coefficients (25x faster than recalculating fully) 
;
; EXAMPLES:  
;
...
; u1 = 0.25d0
; u1 = 0.75d0
; occultquad_vec, z, u1, u2, p, muo1, muo, lambdad, etad
; u1 = 0.35
; u2 = 0.55
; muo2 =1.d0-((1.d0-u1-2.d0*u2)*(1-mu0)+(u1+2.d0*u2)*$
;            (lambdad+2.d0/3.d0*(p gt z))+u2*etad)/(1.d0-u1/3.d0-u2/6.d0)
;
...
;    allow negative values of p (positive transit) to avoid Lutz-Kelker type bias
...
;if p0 lt 0 then begin
;    muo1 = !values.d_infinity
;    mu0 = !values.d_infinity
;    return
;endif
...
;; (please report it)
if notused5[0] ne -1 then message, $
  "ERROR: the following values of z didn't fit into a case:" +$
